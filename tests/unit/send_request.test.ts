...
describe("send_request", () => {
	it("should send the correct data for validation", async () => {
		await send_request("secret", "id", "my-event", "test description", "https://www.google.com", true)
		const expectedData = {
			client_id: "github",
	...
	it("should send the correct data", async () => {
		await send_request("secret", "id", "my-event", "test description", "https://www.google.com", false)
		const expectedData = {
			client_id: "github",
			non_personalized_ads: true,
			events: [{
